...
public class WordUtils {
    ...
    public static String abbreviate(final String str, int lower, int upper, final String appendToEnd) {
        Validate.isTrue(upper >= 0, "upper value cannot be less than 0");
        Validate.isTrue(upper >= lower, "upper value is less than lower value");
    ...
    public static String capitalize(final String str, final char... delimiters) {
        ...
        final Set<Integer> ds = generateDelimiterSet(delimiters);
        final int sl = str.length();
        final int[] newCodePoints = new int[sl];
        int outOffset = 0;

        boolean capitalizeNext = true;
        for (int index = 0; index < sl;) {
            final int codePoint = str.codePointAt(index);

            if (ds.contains(codePoint)) {
                capitalizeNext = true;
    ...
    public static String initials(final String str, final char... delimiters) {
        ...
        final Set<Integer> ds = generateDelimiterSet(delimiters);
        final int sl = str.length();
        final int[] newCodePoints = new int[sl / 2 + 1];
        int count = 0;
        boolean lastWasGap = true;
        for (int i = 0; i < sl;) {
            final int codePoint = str.codePointAt(i);

            if (ds.contains(codePoint) || delimiters == null && Character.isWhitespace(codePoint)) {
                lastWasGap = true;
            } else if (lastWasGap) {
    ...
    public static String swapCase(final String str) {
        ...
        for (int index = 0; index < sl;) {
            ...
            if (Character.isUpperCase(oldCodepoint) || Character.isTitleCase(oldCodepoint)) {
                ...
                ws = Character.isWhitespace(oldCodepoint);
